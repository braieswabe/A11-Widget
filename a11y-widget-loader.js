...
try {
  console.log('[A11Y Loader] ✅ Script file loaded - v1.6.1 -', new Date().toISOString());
  ...
  console.log('[A11Y Loader] If you see this, the loader script v1.6.1 is executing');
} catch(e) {
...
(function() {
  ...
  function loadWidget() {
    ...
    // #region agent log
    try {
      fetch('http://127.0.0.1:7244/ingest/3544e706-ca53-43b1-b2c7-985ccfcff332',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'a11y-widget-loader.js:302',message:'CSS URL construction',data:{CDN_BASE:CDN_BASE,WIDGET_FILES_VERSION:WIDGET_FILES_VERSION,cssUrl:cssUrl,WIDGET_VERSION_TAG:WIDGET_VERSION_TAG},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
    } catch(e) {}
    // #endregion
    cssLink.href = cssUrl;
    ...
    // #region agent log
    try {
      fetch('http://127.0.0.1:7244/ingest/3544e706-ca53-43b1-b2c7-985ccfcff332',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'a11y-widget-loader.js:334',message:'JS URL construction',data:{CDN_BASE:CDN_BASE,WIDGET_FILES_VERSION:WIDGET_FILES_VERSION,scriptUrl:scriptUrl,WIDGET_VERSION_TAG:WIDGET_VERSION_TAG},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
    } catch(e) {}
    // #endregion
  ...
  function checkAuthAndLoad() {
    // Always load widget button so users can see it and click to login
    // Authentication will be checked when button is clicked
    loadWidget();
    ...
    // Load auth module if available (for login on button click)
    if (!window.__a11yAuth) {
      // Load auth module for login functionality
      var authScript = document.createElement("script");
      authScript.src = CDN_BASE + "a11y-widget-auth.js?v=" + WIDGET_FILES_VERSION + "&_=" + Date.now();
      authScript.onload = function() {
        console.log('[A11Y] Auth module loaded. Widget button will prompt for login when clicked.');
      };
      authScript.onerror = function() {
        console.warn('[A11Y] Auth script failed to load. Widget will work without authentication.');
      };
