# QA: updated widget runtime (`a11y-widget-v1.1.0.js` + `a11y-widget.css`)
...
3. Confirm:
   - **Script** response URL ends with **`a11y-widget-v1.1.0.js`** (not legacy `a11y-widget.js`) unless you intentionally test legacy.
   - **Stylesheet** **`a11y-widget.css`** loads with status **200**.
   - Only **one** widget script is injected (no duplicate loaders/runtimes).
4. In the **Console**, run:
   - `window.__A11Y_WIDGET_BUILD__` → should be `"a11y-widget-v1.1.0.js"`.
