  ...
  plugins: [
    react(),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use((req, res, next) => {
          ...
          const isWidgetJsRequest =
            req.url === '/a11y-widget-v1.7.5.js' ||
            req.url?.startsWith('/a11y-widget-v1.7.5.js?') ||
          ...
          if (isWidgetJsRequest) {
            ...
            const widgetJsVersionedRoot = join(__dirname, '..', 'a11y-widget-v1.7.5.js')
      ...
      closeBundle() {
        ...
        const npmCoreJs = join(__dirname, '..', 'packages', 'a11y-widget', 'vendor', 'a11y-widget.core.js')
        const npmCss = join(__dirname, '..', 'packages', 'a11y-widget', 'assets', 'a11y-widget.css')
        const widgetJsVersioned = join(__dirname, '..', 'a11y-widget-v1.7.5.js')
        const widgetLoaderVersioned = join(__dirname, '..', 'a11y-widget-loader-v1.7.5.js')
        ...
        if (existsSync(npmCoreJs)) {
          copyFileSync(npmCoreJs, join(distPath, 'a11y-widget-v1.7.5.js'))
          console.log('[Vite] Copied widget v1.7.5 from npm package')
        } else if (existsSync(widgetJsVersioned)) {
          copyFileSync(widgetJsVersioned, join(distPath, 'a11y-widget-v1.7.5.js'))
          console.log('[Vite] Copied widget v1.7.5 fallback from root')
        }

        if (existsSync(widgetLoaderVersioned)) {
          copyFileSync(widgetLoaderVersioned, join(distPath, 'a11y-widget-loader-v1.7.5.js'))
          console.log('[Vite] Copied widget loader v1.7.5')
        }
