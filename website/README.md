...
**Runtime on this site:** `npm run dev` and production builds load `a11y-widget-v1.7.3.js` and `a11y-widget.css` from the npm package copy in `packages/a11y-widget` (Vite middleware + `closeBundle` copy). Run `npm run sync-widget` before building so the package mirrors the GitHub release assets.
